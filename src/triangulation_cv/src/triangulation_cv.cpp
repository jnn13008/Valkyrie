...
void callback(const openpose_ros_msgs::OpenPoseHumanListConstPtr& second)
{
  if (first_msg == 0){
    ...
    try{
      ...
      float camera_translation_x = abs(transform_now.transform.translation.x-transform_past.transform.translation.x);
      float camera_translation_y = abs(transform_now.transform.translation.y-transform_past.transform.translation.y);
      ...
      points_perspective_one = Mat_<float>::zeros(2, 25);
      points_perspective_two = Mat_<float>::zeros(2, 25);
      for(int i = 0; i < 25; i++) {
      ...
      triangulation_result = Mat_<float>::zeros(4, 25);
    
      //triangulate
      cv::triangulatePoints(projection_matrix, projection_matrix_translation, points_perspective_one, points_perspective_two, triangulation_result);
...
private:
  ...
  int first_msg;
  FileStorage fs("logitechc110.yml", FileStorage::WRITE);
